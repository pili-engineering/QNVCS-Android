...
import com.qiniu.droid.video.cloud.rtc.fragment.ControlFragment;
import com.qiniu.droid.video.cloud.rtc.model.RTCUser;
import com.qiniu.droid.video.cloud.rtc.model.RoomUserList;
import com.qiniu.droid.video.cloud.rtc.model.UserTrack;
import com.qiniu.droid.video.cloud.rtc.ui.CircleTextView;
import com.qiniu.droid.video.cloud.rtc.ui.MergeLayoutConfigView;
import com.qiniu.droid.video.cloud.rtc.ui.UserTrackView;
import com.qiniu.droid.video.cloud.rtc.utils.Config;
import com.qiniu.droid.video.cloud.rtc.utils.QNAppServer;
import com.qiniu.droid.video.cloud.rtc.utils.SplitUtils;
import com.qiniu.droid.video.cloud.rtc.utils.ToastUtils;
import com.qiniu.droid.video.cloud.rtc.utils.TrackWindowMgr;
...
import static com.qiniu.droid.video.cloud.rtc.utils.Config.DEFAULT_BITRATE;
import static com.qiniu.droid.video.cloud.rtc.utils.Config.DEFAULT_FPS;
import static com.qiniu.droid.video.cloud.rtc.utils.Config.DEFAULT_RESOLUTION;
...
public class RoomActivity extends FragmentActivity implements QNRTCEngineEventListener, ControlFragment.OnCallEvents {
    ...
    private boolean mByteDanceEffectEnable = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.rtc_activity_muti_track_room);
        ...
        if (mByteDanceEffectEnable) {
            //第三方美颜
            mEffectManager = new ByteDancePluginManager(this, false);
            mEngine.setCaptureVideoCallBack(new QNCaptureVideoCallback() {
                @Override
                public void onCaptureStarted() {
                    mEffectManager.init(null);
                }

                @Override
                public void onRenderingFrame(VideoFrame.TextureBuffer textureBuffer, long l) {
                    int outTextureId = mEffectManager.processTexture(textureBuffer.getTextureId(),
                            textureBuffer.getWidth(),
                            textureBuffer.getHeight(),
                            textureBuffer.getType() == VideoFrame.TextureBuffer.Type.OES);
                    textureBuffer.setTextureId(outTextureId);
    ...
    private void initQNRTCEngine() {
        ...
        // 2. 如果远端连麦出现回声的现象，可以通过配置 setLowAudioSampleRateEnabled(true) 或者 setAEC3Enabled(true) 后再做进一步测试，并将 model 信息反馈给七牛技术支持
    ...
    private void disconnectWithErrorMessage(final String errorMessage) {
        new AlertDialog.Builder(this)
                .setTitle(getText(R.string.rtc_channel_error_title))
                .setMessage(errorMessage)
                .setCancelable(false)
                .setNeutralButton(R.string.rtc_ok,
    ...
    private void showKickoutDialog(final String userId) {
        if (mKickOutDialog == null) {
            mKickOutDialog = new AlertDialog.Builder(this)
                    .setNegativeButton(R.string.rtc_negative_dialog_tips, null)
                    .create();
        }
        mKickOutDialog.setMessage(getString(R.string.rtc_kickout_tips, userId));
        mKickOutDialog.setButton(DialogInterface.BUTTON_POSITIVE, getResources().getString(R.string.rtc_positive_dialog_tips),
                new DialogInterface.OnClickListener() {
                    @Override
    ...
    public void onRoomStateChanged(QNRoomState state) {
        ...
        switch (state) {
            case RECONNECTING:
                logAndToast(getString(R.string.rtc_reconnecting_to_room));
            ...
            case CONNECTED:
                ...
                logAndToast(getString(R.string.rtc_connected_to_room));
                mIsJoinedRoom = true;
            ...
            case RECONNECTED:
                ...
                break;
            case CONNECTING:
                logAndToast(getString(R.string.rtc_connecting_to, mRoomId));
                break;
        }
    }
    ...
    public void onKickedOut(String userId) {
        ToastUtils.s(RoomActivity.this, getString(R.string.rtc_kicked_by_admin));
        finish();
    }
    ...
    public void onCallStreamingConfig() {
        ...
        if (mPopWindow == null) {
            ...
            mPopWindow.setBackgroundDrawable(new ColorDrawable(ContextCompat.getColor(this, R.color.rtc_popupWindowBackground)));
        }
    ...
    private class UserListAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new ViewHolder(LayoutInflater.from(getApplicationContext()).inflate(R.layout.rtc_item_user, parent, false));
        }
        ...
        public void onBindViewHolder(final ViewHolder holder, int position) {
            ...
            if (mChooseUser != null && mChooseUser.getUserId().equals(userId)) {
                holder.itemView.setBackground(getResources().getDrawable(R.drawable.rtc_white_background));
            } else {
